...
test('test', async ({ page }) => {
  ...
  await writeFile(join(root, 'perf-results', 'reducer-navigation-timing.json'), JSON.stringify(navigationTimingJson))

  const resourceTimingJson = await page.evaluate(() => performance.getEntriesByType('resource'))

  await writeFile(join(root, 'perf-results', 'reducer-resource-timing.json'), JSON.stringify(resourceTimingJson))

  const paintTimingJson = await page.evaluate(() => performance.getEntriesByType('paint'))
  await writeFile(join(root, 'perf-results', 'reducer-paint-timing.json'), JSON.stringify(paintTimingJson))
  ...
  await writeFile(join(root, 'perf-results', 'reducer-lcp.json'), JSON.stringify(largestContentfulPaint))
  ...
  await writeFile(join(root, 'perf-results', 'reducer-cls.json'), JSON.stringify(cummulativeLayoutShift))
  ...
  await writeFile(join(root, 'perf-results', 'reducer-total-blocking-time.json'), JSON.stringify(totalBlockingTime))
})
