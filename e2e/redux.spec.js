...
test('test', async ({ page }) => {
  ...
  await writeFile('redux-navigation-timing.json', JSON.stringify(navigationTimingJson))

  const resourceTimingJson = await page.evaluate(() => performance.getEntriesByType('resource'))

  await writeFile('redux-resource-timing.json', JSON.stringify(resourceTimingJson))

  const paintTimingJson = await page.evaluate(() => performance.getEntriesByType('paint'))
  await writeFile('redux-paint-timing.json', JSON.stringify(paintTimingJson))
  ...
  await writeFile('redux-lcp.json', JSON.stringify(largestContentfulPaint))
  ...
  await writeFile('redux-cls.json', JSON.stringify(cummulativeLayoutShift))
  ...
  await writeFile('redux-total-blocking-time.json', JSON.stringify(totalBlockingTime))
})
