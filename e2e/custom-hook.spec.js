...
test('test', async ({ page }) => {
  ...
  await writeFile('custom-hook-navigation-timing.json', JSON.stringify(navigationTimingJson))

  const resourceTimingJson = await page.evaluate(() => performance.getEntriesByType('resource'))

  await writeFile('custom-hook-resource-timing.json', JSON.stringify(resourceTimingJson))

  const paintTimingJson = await page.evaluate(() => performance.getEntriesByType('paint'))
  await writeFile('custom-hook-paint-timing.json', JSON.stringify(paintTimingJson))
  ...
  await writeFile('custom-hook-lcp.json', JSON.stringify(largestContentfulPaint))
  ...
  await writeFile('custom-hook-cls.json', JSON.stringify(cummulativeLayoutShift))
  ...
  await writeFile('custom-hook-total-blocking-time.json', JSON.stringify(totalBlockingTime))
})
